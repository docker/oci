...
// Descriptor implements [oci.BlobReader.Descriptor].
func (r *bytesReader) Descriptor() oci.Descriptor {
	return r.desc
}
...
func (b *Buffer) GetBlob() (oci.Descriptor, []byte, error) {
	...
	defer b.mu.Unlock()
	if !b.committed {
		return oci.Descriptor{}, nil, fmt.Errorf("blob not committed")
	}
	if b.commitErr != nil {
		return oci.Descriptor{}, nil, b.commitErr
	}
...
func (b *Buffer) Write(data []byte) (int, error) {
	...
	if offset := b.checkStartOffset; offset != -1 {
		...
		if int64(len(b.buf)) != offset {
			return 0, fmt.Errorf("invalid offset %d in resumed upload (actual offset %d): %w", offset, len(b.buf), oci.ErrRangeInvalid)
		}
...
// ID implements [oci.BlobWriter.ID] by returning a randomly
// allocated hex UUID.
func (b *Buffer) ID() string {
	return b.uuid
}

// Commit implements [oci.BlobWriter.Commit] by checking
...
func (b *Buffer) checkCommit(dig oci.Digest) (err error) {
	b.mu.Lock()
	defer b.mu.Unlock()
	if b.commitErr != nil {
		return b.commitErr
