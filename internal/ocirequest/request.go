...
var (
	errBadlyFormedDigest = oci.NewError("badly formed digest", oci.ErrDigestInvalid.Code(), nil)
...
func httpErrorf(statusCode int, f string, a ...any) error {
	return oci.NewHTTPError(fmt.Errorf(f, a...), statusCode, nil, nil)
}
...
//
// If it returns an error, it will be of type [oci.Error] or [oci.HTTPError].
//
...
func Parse(method string, u *url.URL) (*Request, error) {
	...
	if !ok {
		return nil, oci.NewError("unknown URL path", oci.ErrNameUnknown.Code(), nil)
	}
	if path == "_catalog" {
		if method != "GET" {
